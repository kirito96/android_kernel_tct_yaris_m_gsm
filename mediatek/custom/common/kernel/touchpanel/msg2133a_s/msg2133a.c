...
static int drvTP_read_info_dwiic_c33 ( void )
{
  ...
  //  dwiic_tx_data[4] = 0x00;
    ...
    if((fw_major_version+fw_minor_version)==0 || (fw_major_version+fw_minor_version)>32)
    {
	//fw_major_version=1; //default test for MT -------------------------------------llf
	//fw_minor_version=2;
	//fw_major_version=5; //default test for each -------------------------------------llf
	//fw_minor_version=0;
	TP_DEBUG("The last block infomation is not already write the FW! \n");
    }
			...
			if (touchData.nTouchKeyCode == 4)
				{
					...
					touchData.Point[0].X=53;     //change key code to point ABS by wwl
...
static int FW_repair_detect (void)
{
    ...
    printk("[MSG2133A]--------------------------------------FW_repair_detect\n");
    mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
...
static int Upgrade_detect()
{
	...
	printk("[MSG2133A]--------------------------------------Upgrade_detect\n");
	...
	/*
		else if(fw_major_version==1) //mutton
		{
			TPMSG="EACH";
