...
{
#ifndef MTK_AUDIO_HD_REC_SUPPORT
#if defined(MTK_DUAL_MIC_USER_CONTROL_FEATURE) && defined(MTK_DUAL_MIC_SUPPORT)
...
#if defined(MTK_ACF_AUTO_GEN_SUPPORT)
#define TONE_FRAME_COUNT           6
static void * acfFindF0Thread(void *arg)
{
	AudioParamTuning *pTuning = (AudioParamTuning *)arg;	
    android_audio_legacy::AudioStreamOut *streamOutput = NULL;
    int ret = 0, adc_value = 0, center_freq = 0, counter = 0;
    uint32_t adc_min_freq = 0, adc_min_vol = 0, hwBufferSize;
    char *outBuffer = NULL;
    bool adc_status;
    int format;
    ...
    pthread_mutex_lock(&pTuning->acf_mutex);

	    // Adjust thread priority
    prctl(PR_SET_NAME, (unsigned long)"acfFindF0Thread", 0, 0, 0);
    setpriority(PRIO_PROCESS, 0, ANDROID_PRIORITY_AUDIO);
	
    pthread_cond_signal(&pTuning->acf_cond); // wake all thread
    pthread_mutex_unlock(&pTuning->acf_mutex);
    ...
    ********************************************/
    ...
    pTuning->getHardwarePointer()->SetAudioCommand(SET_CUREENT_SENSOR_ENABLE,1);
    format = android_audio_legacy::AudioSystem::PCM_16_BIT;
    channels = android_audio_legacy::AudioSystem::CHANNEL_OUT_STEREO;
    sampleRate = 44100;
    streamOutput = pTuning->getHardwarePointer()->openOutputStream(0, &format, &channels, &sampleRate, &status);
	
	hwBufferSize = streamOutput->bufferSize(); //16k bytes
    outBuffer = new char[hwBufferSize];
    memset(outBuffer, 0, hwBufferSize);
    streamOutput->write(outBuffer, hwBufferSize);
    streamOutput->write(outBuffer, hwBufferSize);
    streamOutput->write(outBuffer, hwBufferSize);
    streamOutput->write(outBuffer, hwBufferSize);
	SXLOGD("pTuning->m_bFindFoThreadExit is %d", pTuning->m_bFindFoThreadExit);
    while(!pTuning->m_bFindFoThreadExit)
    {
        AudioFtm::getInstance()->SetVibSpkEnable(true, center_freq);
        if(counter == 0)
            AudioFtm::getInstance()->SetVibSpkRampControl(2);
        streamOutput->write(outBuffer, hwBufferSize);
        counter++;
        if(counter == TONE_FRAME_COUNT)
        {
            pTuning->getHardwarePointer()->SetAudioCommand(SET_CURRENT_SENSOR_RESET,1);
            usleep(1000);
            pTuning->getHardwarePointer()->SetAudioCommand(SET_CURRENT_SENSOR_RESET,0);
            usleep(5000);

#if 0
            adc_status = pTuning->getHardwarePointer()->ReadAuxadcData(16, &adc_value);
            int32_t adc_sum = adc_value;
            adc_status = pTuning->getHardwarePointer()->ReadAuxadcData(16, &adc_value);
            adc_sum += adc_value;
            adc_status = pTuning->getHardwarePointer()->ReadAuxadcData(16, &adc_value);
            adc_sum += adc_value;
            adc_status = pTuning->getHardwarePointer()->ReadAuxadcData(16, &adc_value);
            adc_sum += adc_value;
            adc_value = (adc_sum + 2)/4;
#else
            adc_status = AudioFtm::getInstance()->ReadAuxadcData(16, &adc_value);
#endif
            ...
            ALOGD("VSCurrentSensor= %d %x at freq %d",adc_value, adc_status, center_freq);
            AudioFtm::getInstance()->SetVibSpkRampControl(1);
            streamOutput->write(outBuffer, hwBufferSize);
            counter = 0;
            center_freq += DELTA_FREQ_FINE;
            if(center_freq >= CENTER_FREQ_END)
            {
                pTuning->mFO_Value = adc_min_freq;
                break;
            }
        }
    }
    pTuning->getHardwarePointer()->SetAudioCommand(SET_CUREENT_SENSOR_ENABLE,0);
    AudioFtm::getInstance()->SetVibSpkEnable(false, 0);
    streamOutput->standby();
    pTuning->getHardwarePointer()->closeOutputStream(streamOutput);
    if(outBuffer)
    {
        delete[] outBuffer;
        outBuffer = NULL;
	...
	//exit thread
    pthread_mutex_lock(&pTuning->acf_mutex);
    SXLOGD("pthread_cond_signal(&acf_cond)");
    pthread_cond_signal(&pTuning->acf_cond); // wake all thread
    pthread_mutex_unlock(&pTuning->acf_mutex);
    return NULL;
}
...
#endif
#if defined(MTK_ACF_AUTO_GEN_SUPPORT)
    ret = pthread_mutex_init(&acf_mutex, NULL);
    if (ret != 0)
    {
        SXLOGE("Failed to initialize acf_mutex!");
    }
    ret = pthread_cond_init(&acf_cond, NULL);
    if (ret != 0)
    {
        SXLOGE("Failed to initialize acf_cond!");
    }
	threadInitialized = false;
	m_bFindFoThreadExit = false;
...
#if defined(MTK_ACF_AUTO_GEN_SUPPORT)
status_t AudioParamTuning::DoACFFindF0Process(bool bEnable)
{
    int ret = 0;
    pthread_t acf_tid_audio;
    struct timeval now;
    ...
    if(bEnable && threadInitialized == false)
    {
        mFO_Value = -1;
        ret = pthread_create(&acf_tid_audio, NULL, acfFindF0Thread, (void *)this);
        if (ret != 0) {
            SXLOGE("Fail to create acfFindF0Thread errno:%d", errno);
            pthread_mutex_destroy(&acf_mutex);
            pthread_cond_destroy(&acf_cond);
            return BAD_VALUE;
        }
		
        pthread_mutex_lock(&acf_mutex);
        SXLOGD("+acf_cond wait");
        ret = pthread_cond_timedwait(&acf_cond,&acf_mutex, &timeout);
        threadInitialized = true;
        SXLOGD("-acf_cond receive ret=%d",ret);
        pthread_mutex_unlock(&acf_mutex);
    }
	else if( !bEnable && threadInitialized )
	{
		SXLOGD("close acfFindF0Thread threadloop");
		pthread_mutex_lock(&acf_mutex);
		
        if(!m_bFindFoThreadExit) {
            m_bFindFoThreadExit = true;
            SXLOGD("+acf_cond wait");
            ret = pthread_cond_timedwait(&acf_cond,&acf_mutex, &timeout);
            SXLOGD("-acf_cond receive ret=%d",ret);
        }
    ...
    {
        SXLOGD("The DoACFFindF0Process is error, bEnable=%d, playing=%d", bEnable,threadInitialized);
        return BAD_VALUE;
    }
