...
status_t AudioMTKHardware::SetAudioCommand(int par1, int par2)
{
    ...
    switch (par1)
    {
		...
		case AUDIO_ACF_FO:
		{
			if(par2 == 1)	// start to create FO thread
			{
				mAudioTuningInstance->DoACFFindF0Process(true);
			}
			else if(par2 == 0)	//stop FO thread
			{
				mAudioTuningInstance->DoACFFindF0Process(false);
			}
			break;
		}
#endif
