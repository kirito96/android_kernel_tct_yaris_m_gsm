...
ImgSensorDrv::impSearchSensor(pfExIdChk pExIdChkCbf)
{
    ...
    static MUINT32 i;
    MUINT32	id[KDIMGSENSOR_MAX_INVOKE_DRIVERS] = {0,0};
...
{ 
    ...
    /*
    case CMD_SENSOR_GET_YUV_SENSOR_CAPTURE_OUTPUT_JPEG:
        FeatureId = SENSOR_FEATURE_GET_YUV_CAPTURE_OUTPUT_JPEG;
        u4FeaturePara[0] = (MUINT32)parg1; 
        FeatureParaLen = sizeof(MUINT32); 
        pFeaturePara = (MUINT8*)parg1;  
        LOG_MSG("TRIGGER_FLASHLIGHT=0x%x\n", parg1);
    ...
    */
    ...
    case CMD_SENSOR_SET_YUV_JPEG_PARA:
        FeatureId = SENSOR_FEATURE_SET_YUV_JPEG_PARA;
        u4FeaturePara[0] = (MUINT32)parg1; 
        FeatureParaLen = sizeof(MUINT32); 
        pFeaturePara = (MUINT8*)&u4FeaturePara[0];//(MUINT8*)parg1;
        break;


    case CMD_SENSOR_GET_YUV_JPEG_INFO:
        FeatureId = SENSOR_FEATURE_GET_YUV_JPEG_INFO;
        u4FeaturePara[0] = (MUINT32)parg1;
        u4FeaturePara[1] = (MUINT32)parg2;
        FeatureParaLen = sizeof(MUINT32)*2; 
        pFeaturePara = (MUINT8*)&u4FeaturePara[0];
...
ImgSensorDrv::customerInit(void)
{
    ...
    if (fp == NULL)
    {
        LOG_MSG("No Customer Sensor Init File Exist \n"); 
        return; 
    }
