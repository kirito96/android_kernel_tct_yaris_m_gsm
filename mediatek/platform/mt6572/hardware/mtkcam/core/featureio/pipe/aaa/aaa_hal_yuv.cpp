...
MBOOL Hal3AYuv::sendCommand(ECmd_T const eCmd, MINT32 const i4Arg)
{
    ...
    else if (eCmd == ECmd_PrecaptureStart)
    {
		...
		m_rYuvFlParam.i4FlashlightDuty = NSCamCustom::custom_GetYuvTorchDuty();
	...
	m_rYuvFlParam.i4FlashlightDuty = NSCamCustom::custom_GetYuvFlashlightDuty();
...
MINT32 Hal3AYuv::getFlashFrameNumBeforeCapFrame()
{
    ...
    if (m_pStrobeDrvObj && m_bFlashActive == TRUE)
    {
        i4FrmNum = 1; /*ersen.shang for flashlight 20141121*/
    }
...
MINT32 Hal3AYuv::getHDRCapInfo(Hal3A_HDROutputParam_T &a_strHDROutputInfo)
{
	...
	/*Begin: ersen.shang 20141127 for tack 3 pictures in HDR mode, original value is 2*/
    HDROutputParam.u4OutputFrameNum = 3;
	/*End  : ersen.shang 20141127 for tack 3 pictures in HDR mode, original value is 2*/
