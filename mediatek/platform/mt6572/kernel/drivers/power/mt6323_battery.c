...
UINT32 BattVoltToPercent(UINT16 dwVoltage)
{
    ...
    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "###### 100 <-> voltage : %d ######\r\n", Batt_VoltToPercent_Table[g_fg_battery_id][10].BattVolt);
    }
    
    if(dwVoltage<=Batt_VoltToPercent_Table[g_fg_battery_id][0].BattVolt)
    {
        bPercntResult = Batt_VoltToPercent_Table[g_fg_battery_id][0].BattPercent;
        return bPercntResult;
    }
    else if (dwVoltage>=Batt_VoltToPercent_Table[g_fg_battery_id][10].BattVolt)
    {
        bPercntResult = Batt_VoltToPercent_Table[g_fg_battery_id][10].BattPercent;
        return bPercntResult;
    }
    else
    {        
        VBAT1 = Batt_VoltToPercent_Table[g_fg_battery_id][0].BattVolt;
        bPercnt1 = Batt_VoltToPercent_Table[g_fg_battery_id][0].BattPercent;
        for(m=1;m<=10;m++)
        {
            if(dwVoltage<=Batt_VoltToPercent_Table[g_fg_battery_id][m].BattVolt)
            {
                VBAT2 = Batt_VoltToPercent_Table[g_fg_battery_id][m].BattVolt;
                bPercnt2 = Batt_VoltToPercent_Table[g_fg_battery_id][m].BattPercent;
                break;
            }
            else
            {
                VBAT1 = Batt_VoltToPercent_Table[g_fg_battery_id][m].BattVolt;
                bPercnt1 = Batt_VoltToPercent_Table[g_fg_battery_id][m].BattPercent;    
            }
        }
    }
...
void BAT_thread(void)
{
    ...
    else
    {
        ...
        if(0)
        {
            if ( (BMT_status.bat_charging_state == CHR_TOP_OFF) &&
                 (BMT_status.SOC == 100) && 
                 (BMT_status.bat_vol >= Batt_VoltToPercent_Table[g_fg_battery_id][10].BattVolt) )
            {
                if (Enable_BATDRV_LOG == 1) {
                    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] Battery real full(%ld,%d) and disable charging !\n", 
                            BMT_status.SOC, Batt_VoltToPercent_Table[g_fg_battery_id][10].BattVolt); 
                }
                BMT_status.bat_charging_state = CHR_BATFULL;
                BAT_BatteryFullAction();
                return;
...
{
    int ret_value=1;
    ret_value = Batt_VoltToPercent_Table[g_fg_battery_id][0].BattVolt;
...
{
    int ret_value=1;
    ret_value = Batt_VoltToPercent_Table[g_fg_battery_id][10].BattVolt;
