...
#ifdef PIXI3_4_FEATURE_SUPPORT
	//BEGIN: delete by fangjie, because since EP03 Lot0, EU and US branch both use 2in1 receiver.
	const char PROPERTY_KEY_2IN1SPK_ON[PROPERTY_KEY_MAX] = "ro.audio.g2in1spk.config"; //0:2in1 Speaker, 1: external Receiver
	//END: delete by fangjie, because since EP03 Lot0, EU and US branch both use 2in1 receiver.
#else
...
bool IsAudioSupportFeature(int dFeatureOption)
{
    ...
    int IndexOfAudioConfig = 1; //Added by fangjie , default : 2in1 speaker.
    ...
    switch (dFeatureOption)
    {
		...
		//BEGIN: delete by fangjie, because since EP03 Lot0, EU and US branch both use 2in1 receiver.
			property_get(PROPERTY_KEY_2IN1SPK_ON, stForFeatureUsage, "1"); //"1": default on
	            	IndexOfAudioConfig = atoi(stForFeatureUsage);
			ALOGD("receiver IndexOfAudioConfig = %d\n",IndexOfAudioConfig);

	            	if(IndexOfAudioConfig == 0) // 0: external Receiver, 1: 2in1Spk
	            	{
				bSupportFlg =  false ;
				//ALOGD("receiver false\n");
	            	}
			else
			{
				bSupportFlg =  true ;
		...
		case AUDIO_SUPPORT_DUAL_MIC:
        {
			property_get("ro.config.dualmic.support", stForFeatureUsage, "0"); //whether support the feature of dualmic for HW. [0] dissable [1] enable
			bSupportFlg = (stForFeatureUsage[0] == '0') ? false : true;
            break;
        }
        #endif
