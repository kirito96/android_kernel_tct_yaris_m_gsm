...
#include <cutils/properties.h> //FR444584-add by rifang.zhang.hz@tcl.com
...
static bool isWireHeadSetOn = false; //FR444584 add by rifang.zhang.hz
                                                  ...
                                                  const char *device_address)
{
    ...
    //FR444584 add by rifang.zhang.hz begin
    ...
    //FR444584 add by rifang.zhang.hz end
...
audio_devices_t AudioMTKPolicyManager::getNewDevice(audio_io_handle_t output, bool fromCache)
{
    ...
    //yi.zheng.hz add begin for CR495299:vibrating on ringtone with headset plugged
    if((outputDesc->isUsedByStrategy(STRATEGY_VIBSPK))&&(mForceUse[AudioSystem::FOR_SYSTEM] == AudioSystem::FORCE_SYSTEM_ENFORCED))
    {
        device = getDeviceForStrategy(STRATEGY_VIBSPK, false);
    }else
    //yi.zheng.hz add end	
...
#endif
    } else if (outputDesc->isUsedByStrategy(STRATEGY_MEDIA)||outputDesc->isUsedByStrategy(STRATEGY_VIBSPK)) {/*yi.zheng.hz modify for CR495299:vibrating on ringtone with headset plugged*/
...
AudioMTKPolicyManager::routing_strategy AudioMTKPolicyManager::getStrategy(
     ...
     //[Audio]FR444584 , by rifang.zhang.hz@tcl.com for customization
    ...
    char headset_sound_mode[10];
	 
    // stream to strategy mapping
    switch (stream) {
    //yi.zheng.hz add begin for CR495299:vibrating on ringtone with headset plugged
    case AudioSystem::VIBSPK:
        return STRATEGY_VIBSPK;
    //yi.zheng.hz add end
    case AudioSystem::VOICE_CALL:
    case AudioSystem::BLUETOOTH_SCO:
        return STRATEGY_PHONE;
    case AudioSystem::RING:
	//FR444584 modified by rifang.zhang.hz@tcl.com for customization begin
	...
	//FR444584 modified by rifang.zhang.hz@tcl.com for customization end
	break;
	...
	//FR444584modified by rifang.zhang.hz@tcl.com for customization end
	break;
                                                             ...
                                                             bool fromCache)
{
    ...
    //yi.zheng.hz add begin for CR495299:vibrating on ringtone with headset plugged
    case STRATEGY_VIBSPK:
        if(!isInCall() && mForceUse[AudioSystem::FOR_SYSTEM] == AudioSystem::FORCE_SYSTEM_ENFORCED)
        {
            //(mPhoneState == AudioSystem::MODE_RINGTONE)
            device = mAvailableOutputDevices & AudioSystem::DEVICE_OUT_SPEAKER;
            ALOGD("force to vibrating1"); 
        }
        else
    ...
    case STRATEGY_PHONE:
        ...
        // of priority
        //yi.zheng.hz add begin for CR495299:vibrating on ringtone with headset plugged
        if((mPhoneState == AudioSystem::MODE_RINGTONE)&&(mForceUse[AudioSystem::FOR_SYSTEM] == AudioSystem::FORCE_SYSTEM_ENFORCED))
        {
            device = mAvailableOutputDevices & AUDIO_DEVICE_OUT_SPEAKER;
            ALOGD("force to vibrating2");
            break;
        }
        //yi.zheng.hz add end
    ...
    case STRATEGY_MEDIA: {
        ...
        //yi.zheng.hz add begin for CR495299:vibrating on ringtone with headset plugged
        if((mForceUse[AudioSystem::FOR_SYSTEM] == AudioSystem::FORCE_SYSTEM_ENFORCED)&&(strategy == STRATEGY_SONIFICATION) && (mPhoneState == AudioSystem::MODE_RINGTONE) &&
			((mAvailableOutputDevices & AUDIO_DEVICE_OUT_WIRED_HEADSET) || (mAvailableOutputDevices & AUDIO_DEVICE_OUT_WIRED_HEADPHONE)))
        {
		//device &= ~(AUDIO_DEVICE_OUT_WIRED_HEADSET | AUDIO_DEVICE_OUT_WIRED_HEADPHONE);
		device = mAvailableOutputDevices & AUDIO_DEVICE_OUT_SPEAKER;
	}
        //yi.zheng.hz add end
        if (device) break;
