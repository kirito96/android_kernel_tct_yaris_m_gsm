...
AudioMATVController::AudioMATVController()
{
    ...
    mIsMatvDirectConnectionMode = true;
#else
...
#if defined(NMI_MATV_AUDIO_SUPPORT)
status_t AudioMATVController::ChangeDevice(const audio_devices_t new_device)
{
    Mutex::Autolock _l(mLock);

    const audio_devices_t pre_device = (audio_devices_t)mAudioMatvResourceManager->getDlOutputDevice();
    ...
    else if (//(pre_device == (AUDIO_DEVICE_OUT_SPEAKER | AUDIO_DEVICE_OUT_WIRED_HEADSET) ||
              //pre_device == (AUDIO_DEVICE_OUT_SPEAKER | AUDIO_DEVICE_OUT_WIRED_HEADPHONE)) ||
             (new_device == (AUDIO_DEVICE_OUT_SPEAKER | AUDIO_DEVICE_OUT_WIRED_HEADSET) ||
              new_device == (AUDIO_DEVICE_OUT_SPEAKER | AUDIO_DEVICE_OUT_WIRED_HEADPHONE)))
    {
        ALOGD("%s(), entering / leaving Warning Tone, only config analog part", __FUNCTION__);
        mAudioMatvResourceManager->SetMatvAFEConnection(false);
    }else{
        mAudioMatvResourceManager->SetMatvAFEConnection(true);
    }
    ...
    // Update
    mAudioMatvResourceManager->setDlOutputDevice(new_device);
