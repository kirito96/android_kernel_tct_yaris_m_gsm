...
//CR 595610, tian.shi, begin
#include <sys/time.h>
//CR 595610, tian.shi, end
...
static bool isWireHeadSetOn = false; //PR766139 add by wei.li.hz@tcl.com

// shouldn't need to touch these
static const float dBConvert = -dBPerStep * 2.302585093f / 20.0f;
static const float dBConvertInverse = 1.0f / dBConvert;

static int64_t kCountMaxTimeUs = 72000000000ll; // 20hrs //CR 595610, tian.shi
                                                  ...
                                                  const char *device_address)
{
    ...
    //FR766139 add by wei.li.hz@tcl.com begin
    if ((device == AUDIO_DEVICE_OUT_WIRED_HEADPHONE || device == AUDIO_DEVICE_OUT_WIRED_HEADSET)
         && state == AudioSystem::DEVICE_STATE_AVAILABLE) {
        isWireHeadSetOn = true;
    } else {
        isWireHeadSetOn = false;
    }
    //FR766139 add by wei.li.hz@tcl.com end
    ...
    if (strcmp(property, "ro.camera.sound.forced") == 0) {
        if (atoi(value)) {
            ...
            if(!strncmp(soundPlayValue, "true", 4)) {
                ALOGD("ENFORCED_AUDIBLE can not be muted");
                mStreams[AudioSystem::ENFORCED_AUDIBLE].mCanBeMuted = false;
            } else {
                                             ...
                                             int session)
{
    ...
    //CR 595610, tian.shi, begin
    if((stream == AudioSystem::MUSIC || stream == AudioSystem::FM)
		&&(outputDesc->mDevice&(AUDIO_DEVICE_OUT_WIRED_HEADSET|AUDIO_DEVICE_OUT_WIRED_HEADPHONE|AUDIO_DEVICE_OUT_BLUETOOTH_A2DP_HEADPHONES))){
        mEverStarted = true;
	 ALOGE("---AudioMTKPolicyManager startOutput set mEverStarted true");
        if(mNeedCount&&startTime==0){
            startTime = systemTime() / 1000;
            ALOGE("---AudioMTKPolicyManager startOutput startTime %lld", startTime);
        }
    }
    //CR 595610, tian.shi, end
...
//remove by wanlin.qin for safe volume count begin
#if 0
    //CR 595610, tian.shi, begin
    if((stream == AudioSystem::MUSIC || stream == AudioSystem::FM)){
        mEverStarted = false;
        if(mNeedCount && startTime != 0){
	        ...
	        ALOGE("---AudioMTKPolicyManager stopOutput endTime %lld", endTime);
	        mTimeCount += (endTime - startTime);
	        startTime = 0;
	        endTime = 0;
        }
    }
...
//add by wanlin.qin for safe volume count begin
if ((!isStreamActive(AudioSystem::MUSIC)) && (!isStreamActive(AudioSystem::FM))) {
	if((stream == AudioSystem::MUSIC || stream == AudioSystem::FM)){
        	mEverStarted = false;
		/*ALOGE("---AudioMTKPolicyManager stopOutput set mEverStarted false");
        	if(mNeedCount && startTime != 0){
	        endTime = systemTime() / 1000;
...
//add by wanlin.qin for safe volume count end
        return NO_ERROR;
    } else {
...
//CR 595610, tian.shi, begin
static int flag_called = 0;//this flag makes sure that startCount() and stopCount() get called in pair.
void AudioMTKPolicyManager::startCount()
{
    ALOGE("---AudioMTKPolicyManager startCount ");
    if(flag_called != 0)
        return;
    flag_called++;
    ...
    if(!mNeedCount)
        mNeedCount = true;
    if(startTime == 0 && mEverStarted){
        startTime = systemTime() / 1000;
    }
    ALOGE("---AudioMTKPolicyManager startCount mNeedCount=%d,startTime=%lld,mEverStarted=%d",mNeedCount,startTime,mEverStarted);
}

void AudioMTKPolicyManager::stopCount()
{
    ALOGE("---AudioMTKPolicyManager stopCount ");
    if(flag_called != 1)
        return;
    flag_called--;
	...
	startTime = 0;
       endTime = 0;
    ALOGE("---AudioMTKPolicyManager stopCount mNeedCount=%d,startTime=%lld,endTime=%lld,mEverStarted=%d",mNeedCount,startTime,endTime,mEverStarted);

}

bool AudioMTKPolicyManager::checkIfExceed()
{
    if(startTime !=0 && endTime ==0){
        stopCount();
        startCount();
    }
    ALOGE("---AudioMTKPolicyManager checkIfExceed, mTimeCount %lld (Secs)!", mTimeCount/(1000*1000));
    if(mTimeCount > kCountMaxTimeUs){
        ALOGE("---AudioMTKPolicyManager exceeded!");
        //mNeedCount = false;
        mTimeCount = 0;
        return true;
    }
    ...
    mSpeakerDrcEnabled(false)
{
    ...
    char audioDetectDelayMin[PROPERTY_VALUE_MAX];
    property_get("audio_detect_delay_minutes", audioDetectDelayMin, "1200");
    kCountMaxTimeUs = atoll(audioDetectDelayMin) * 60 * 1000 * 1000;
}
...
AudioMTKPolicyManager::routing_strategy AudioMTKPolicyManager::getStrategy(
     ...
     //[Audio]PR797748 modified by wei.li.hz@tcl.com for customization
     //  ro.headset.sound.mode = 0, disable
     //  ro.headset.sound.mode = 1, headset only
     //  ro.headset.sound.mode = 2, both the headset and the speaker
     //also it must be decide by end user in setting
    char headset_sound_mode[100];
	...
	//PR797748 modified by wei.li.hz@tcl.com for customization begin
	property_get("persist.sys.headset_mode",headset_sound_mode,"1");
				
	if (atoi(headset_sound_mode) == 0)
	{
	    return STRATEGY_SONIFICATION;
        } else {
             if (atoi(headset_sound_mode) == 1 && isWireHeadSetOn){
                   return STRATEGY_PHONE;
             } else if (atoi(headset_sound_mode) == 2){
                   return STRATEGY_SONIFICATION;
	     } else {
	           return STRATEGY_SONIFICATION;
	     }
        }
	//PR797748 modified by wei.li.hz@tcl.com for customization end
                                                             ...
                                                             bool fromCache)
{
    ...
    //PR 595610, tian.shi, if FM is put out from speaker, we'll get device again, rather than from cache, begin
    if (fromCache && !mFmForceSpeakerState) {
        ALOGVV("getDeviceForStrategy() from cache strategy %d, device %x",
              strategy, mDeviceForStrategy[strategy]);
        return mDeviceForStrategy[strategy];
    }
    //PR 595610, tian.shi, end
    ...
    case STRATEGY_PROPRIETARY_FM: {
        ...
        //PR 595610, tian.shi, if FM comes from speaker, we'll need the right output device. begin
        if((device2 == 0) && STRATEGY_PROPRIETARY_FM==strategy && mFmForceSpeakerState){
                                             ...
                                             int delayMs)
{
	...
	//add by wanlin.qin for safe volume count begin
	char fmVolume[100];
	char musicVolume[100];
	property_get("ro.audio.safety.fm.value",fmVolume,"11");
	property_get("ro.audio.safety.music.value",musicVolume,"11");
	ALOGD("setOutputDevice:fmVolume=%s,musicVolume=%s",fmVolume,musicVolume);	
	ALOGD("setOutputDevice:FM current volume=%d,music current volume=%d",mStreams[AudioSystem::FM].getVolumeIndex(device),mStreams[AudioSystem::MUSIC].getVolumeIndex(device));
	if(device&(AUDIO_DEVICE_OUT_WIRED_HEADSET|AUDIO_DEVICE_OUT_WIRED_HEADPHONE|AUDIO_DEVICE_OUT_BLUETOOTH_A2DP_HEADPHONES)
		&&(isStreamActive(AudioSystem::MUSIC)||isStreamActive(AudioSystem::FM))){
        		mEverStarted = true;
			ALOGE("setOutputDevice:set mEverStarted true");
			if(startTime==0&&((isStreamActive(AudioSystem::FM)&&mStreams[AudioSystem::FM].getVolumeIndex(device)>=atoi(fmVolume))
				||(isStreamActive(AudioSystem::MUSIC)&&mStreams[AudioSystem::MUSIC].getVolumeIndex(device)>=atoi(musicVolume)))){
            		startTime = systemTime() / 1000;
			mNeedCount=true;
            		ALOGE("---AudioMTKPolicyManager setOutputDevice startTime %lld", startTime);
        		}else if(!((isStreamActive(AudioSystem::FM)&&mStreams[AudioSystem::FM].getVolumeIndex(device)>=atoi(fmVolume))
				||(isStreamActive(AudioSystem::MUSIC)&&mStreams[AudioSystem::MUSIC].getVolumeIndex(device)>=atoi(musicVolume)))
				 && startTime != 0){				 
				endTime = systemTime() / 1000;
	        		ALOGE("---AudioMTKPolicyManager setOutputDevice mEverStarted true endTime %lld", endTime);
	        		mTimeCount += (endTime - startTime);	       
				startTime = 0;
	        		endTime = 0;
				mNeedCount=false;
	       		 ...
	       		 mEverStarted = false;
			 ALOGE("setOutputDevice:set mEverStarted false");
        	if(mNeedCount && startTime != 0){
	        	endTime = systemTime() / 1000;
	        	ALOGE("---AudioMTKPolicyManager setOutputDevice mEverStarted false endTime %lld", endTime);
	        	mTimeCount += (endTime - startTime);	        	
        	}
			startTime = 0;
	        	endTime = 0;
                                                   ...
                                                   bool force)
{
	...
	//add by wanlin.qin for DT132956 begin 
	ALOGD("PR926829    ");
     if ( (stream == AudioSystem::MUSIC) && (mPhoneState ==AudioSystem::MODE_RINGTONE) ) {
	 	ALOGD("PR926829 l music is playing ,and rington is active,so need set music mute!!!  ");
         volume =0.0;
    }
	 //add by wanlin.qin for DT132956 end
    // ALPS001125976 Mute music at ringtone from BT to primary
