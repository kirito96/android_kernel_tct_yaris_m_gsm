...
extern "C" {
    ...
    static void ap_start_count(const struct audio_policy *pol)
    {
        const struct legacy_audio_policy *lap = to_clap(pol);
        lap->apm->startCount();
        return;
    ...
    static void ap_stop_count(const struct audio_policy *pol)
    {
        const struct legacy_audio_policy *lap = to_clap(pol);
        lap->apm->stopCount();
        return;
    ...
    static bool ap_check_if_exceed(const struct audio_policy *pol)
    {
        const struct legacy_audio_policy *lap = to_clap(pol);
        return lap->apm->checkIfExceed();
    }
                                   ...
                                   const audio_offload_info_t *info)
{
                                ...
                                struct audio_policy **ap)
    {
        ...
        //CR 595610, tian.shi, begin
        lap->policy.start_count = ap_start_count;
        lap->policy.stop_count = ap_stop_count;
        lap->policy.check_if_exceed = ap_check_if_exceed;
        //CR 595610, tian.shi, end
